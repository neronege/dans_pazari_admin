...
export default function BlogCategoriesPage() {
  ...
  const submitForm = async () => {
    ...
    if (!String(root.name || '').trim()) {
      setFormErrors({ 'translations.tr.name': 'Kategori adı zorunludur.' });
      setLocaleTab('tr');
      setSaving(false);
      return;
    }
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} fullWidth maxWidth="sm">
                ...
                return (
                  <Stack sx={{ gap: 2 }}>
                    <TextField
                      ...
                      helperText={withFieldError(getFieldError(formErrors, `translations.${locale}.name`), lengthFieldProps(row.name, FIELD_LIMITS.blogCategory.name).helperText)}
                      {...lengthFieldProps(row.name, FIELD_LIMITS.blogCategory.name)}
                    />
                    <TextField
                      label="Slug"
                      value={row.slug}
        ...
        <DialogActions>
          ...
          <Button
            ...
            disabled={
              saving ||
              translationsHaveLengthErrors(translations, FIELD_LIMITS.blogCategory)
            }
          >
